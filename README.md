...
Apply schema with `pnpm migrate` (advisory-locked; safe if two operators run it). Engines do **not** auto-migrate; they exit if the database is behind the files in `migrations/`.
